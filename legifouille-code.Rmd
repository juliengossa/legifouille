...
codever.tmp <- read.csv2("data/fr-legi-codes-en-vigueur-versions.csv") %>%
  filter(code == lecode) %>%
...
bind_rows(
  ...
  ggplot_sd(aes(x=président,y=Nombre_versions,fill=partie,alpha=Données)) +
...
bind_rows(
  ...
  ggplot_sd(aes(x=président,y=Nombre_modifications,fill=partie,alpha=Données)) +
  geom_col(position = position_stack(reverse = TRUE)) +
...
codever_mandats %>% 
  ...
  ggplot_sd(aes(x=président,y=Nombre_modifications_par_version,fill=partie)) +
  geom_boxplot() +
...
# waffle()
waffle(from="2017-06-01",to=date_update)
```
...
waffle.pngs <- function(lecode="code de l'éducation",
  ...
  gifski::gifski(pngs,paste0("gifs/",lecode,".gif"),width = 800, height = 800, delay = delay)
}

waffle.pngs(from="2017-06-01", to=date_update, width=100, height=57, delay = 0.3, pause=8, keeppng = FALSE)
```
...
scolstats %>%
  ...
  ggplot_sd(aes(x=date,y=`Part des articles`,fill=Etat)) +
  geom_area()
```
...
waffle.scol <- function(width=100, height=0, delay=1, pause=3,
  ...
  pngs <- c(rep(pngs[1],pause),pngs,rep(pngs[length(pngs)],pause*2))
  gifski::gifski(pngs,paste0("gifs/scolarite.gif"),width = 800, height = 800, delay = delay)
}
